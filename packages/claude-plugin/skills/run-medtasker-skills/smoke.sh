#!/usr/bin/env bash
# Smoke test for medtasker-skills CLI.
#
...
#
# Usage:
#   .claude/skills/run-medtasker-skills/smoke.sh         # build + run + cleanup
#   KEEP=1 .claude/skills/run-medtasker-skills/smoke.sh  # leave the temp HOME for inspection
#
# Requires: go, dotenvx (already a runtime dep of medtasker-skills).
set -euo pipefail

# Find the medtasker-skills repo by walking up from the script's real dir
# (resolves symlinks). Falls back to MEDTASKER_REPO. Works whether the script
# is run from packages/, via the .claude/ symlink, or after `medtasker-skills
...
if [[ -z "$REPO_ROOT" || ! -f "$REPO_ROOT/go.mod" ]]; then
  echo "smoke.sh: medtasker-skills source repo not found." >&2
  echo "         Run from inside the repo, or set MEDTASKER_REPO=/path/to/medtasker-skills." >&2
  exit 1
fi
cd "$REPO_ROOT"

# Isolated HOME so we never touch the real ~/.claude or ~/.medtasker-skills
SMOKE_DIR="$(mktemp -d -t mts-smoke.XXXXXX)"
trap '[[ "${KEEP:-0}" == "1" ]] || rm -rf "$SMOKE_DIR"' EXIT
SMOKE_HOME="$SMOKE_DIR/home"
mkdir -p "$SMOKE_HOME/.claude" "$SMOKE_HOME/.medtasker-skills"
chmod 700 "$SMOKE_HOME/.medtasker-skills"

BIN="$SMOKE_DIR/medtasker-skills"
...
assert_contains "medtasker-jira" "$out" "list shows medtasker-jira from embedded packages"
...
section "install medtasker-jira"
out="$(run install medtasker-jira 2>&1)"
assert_contains "1 skills installed" "$out" "install reports success"
assert_file "$SMOKE_HOME/.claude/skills/medtasker-jira/SKILL.md" "skill copied to ~/.claude/skills/"
...
# list (after install -- should mark medtasker-jira ✓) -----------------------
section "list (after install)"
out="$(run list)"
if grep -qE '✓ +medtasker-jira' <<<"$out"; then
  pass "list marks medtasker-jira as installed"
else
  fail "medtasker-jira not marked installed"
fi

# env: full vault round-trip -------------------------------------------------
section "env set / list / encrypt / decrypt"
run env set SMOKE_KEY hunter2-super-secret-value >/dev/null
assert_file "$SMOKE_HOME/.medtasker-skills/.env" "vault .env written"
# dotenvx encrypts on `set`, so .env should contain ciphertext already
if grep -q '^SMOKE_KEY="encrypted:' "$SMOKE_HOME/.medtasker-skills/.env"; then
  pass "value stored as encrypted ciphertext"
else
  fail "value not encrypted in .env"
  cat "$SMOKE_HOME/.medtasker-skills/.env" | sed 's/^/      | /'
fi
# decrypt round-trip
run env decrypt >/dev/null 2>&1 || true
if grep -q '^SMOKE_KEY="hunter2-super-secret-value"' "$SMOKE_HOME/.medtasker-skills/.env"; then
...
grep -q '^SMOKE_KEY="encrypted:' "$SMOKE_HOME/.medtasker-skills/.env" \
