---
name: run-stevmachine-skills
description: Build, run, smoke-test, and drive the `stevmachine-skills` Go CLI and its huh-based `env setup` TUI wizard. Use to verify a change to the CLI, install/list/doctor subcommands, MCP config writer, or vault wrapper, and to capture wizard screenshots.
---

# Run / drive stevmachine-skills

`stevmachine-skills` is a single Go binary that copies embedded skill packages into `~/.claude/skills/`, writes `~/.claude/.mcp.json`, and wraps `dotenvx` for an encrypted credential vault at `~/.stevmachine-skills/`. Two surfaces matter for verifying a change:
...
The skill lives at `packages/claude-plugin/skills/run-stevmachine-skills/` so it gets embedded in the distribution binary; `.claude/skills/run-stevmachine-skills/` is a symlink to it for Claude Code auto-discovery in this repo. Either path works for invocation. Paths below are relative to the repo root.
...
- `env set` writes `~/.stevmachine-skills/.env` with `encrypted:` ciphertext
- `go test ./internal/mcp ./cmd/...` passes

On success: `16 passed, 0 failed`. The temp `$HOME` is wiped on exit; pass `KEEP=1` to inspect.

```bash
KEEP=1 .claude/skills/run-stevmachine-skills/smoke.sh
...
```bash
.claude/skills/run-stevmachine-skills/wizard.sh decline   # walks past every prompt with 'n'
.claude/skills/run-stevmachine-skills/wizard.sh jira      # types dummy Jira creds
.claude/skills/run-stevmachine-skills/wizard.sh all       # Jira + GitHub
```
...
```bash
WIZ=$(mktemp -d); mkdir -p "$WIZ/home/.claude" "$WIZ/home/.stevmachine-skills"
chmod 700 "$WIZ/home/.stevmachine-skills"
go build -o "$WIZ/bin" ./cmd/stevmachine-skills
...
tmux capture-pane -t shot -p > .claude/skills/run-stevmachine-skills/samples/wizard-jira-prompt.txt
...
```bash
go build -o /opt/homebrew/bin/stevmachine-skills ./cmd/stevmachine-skills
stevmachine-skills install        # installs into your real ~/.claude/skills/
stevmachine-skills env setup      # interactive — fill in your tokens
...
go run ./cmd/stevmachine-skills list              # quickest sanity check
```
...
- **`env set` needs the vault dir to exist.** `vault.Manager.Set` runs `dotenvx` with `cmd.Dir = ~/.stevmachine-skills`, but never `MkdirAll`s it (only `InitVault` does, which isn't called from `cmdEnvSet` or `cmdEnvSetup`). On a fresh `$HOME`, both `set` and the wizard fail with `chdir … no such file or directory` until the dir exists. The smoke scripts pre-create it. The CLI itself is missing this step — file an issue rather than working around it in tests.
- **`stevmachine-skills env list` says "No vault initialized" even when it is.** `vault.IsInitialized` checks for `.env.keys`, but dotenvx ≥ 1.66 stores the private key in the system keyring (or `DOTENV_PRIVATE_KEY` env var) and **never writes `.env.keys`** by default. The data is there — `cat ~/.stevmachine-skills/.env` will show the `encrypted:` lines.
...
| Symptom | Fix |
|---|---|
| `chdir … /home/.stevmachine-skills: no such file or directory` | The vault dir doesn't exist. `mkdir -p ~/.stevmachine-skills && chmod 700 ~/.stevmachine-skills`. The smoke scripts already do this. |
| `MISSING_PRIVATE_KEY` from `env decrypt` or `env list` | dotenvx ≥ 1.66 needs the private key in env. `export DOTENV_PRIVATE_KEY=$(dotenvx keypair -f ~/.stevmachine-skills/.env DOTENV_PRIVATE_KEY)` or accept that `env list` won't decrypt — the smoke skips this check. |
